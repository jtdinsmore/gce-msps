...
class LuminosityFunction:
    ...
    def __call__(self, x):
        logx = log10(x)
        i = 0
        while i < len(self.logxs) and self.logxs[i] < logx:
            i+= 1
        if i >= len(self.logxs) - 1:
            return 0;
        frac = (logx - self.logxs[i]) / (self.logxs[i + 1] - self.logxs[i])
        return self.ys[i] + frac * (self.ys[i + 1] - self.ys[i])
